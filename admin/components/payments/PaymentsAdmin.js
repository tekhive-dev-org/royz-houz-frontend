...
export function PaymentsAdmin() {
  ...
  return <Box sx={{ width: "100%", maxWidth: "100%", minWidth: 0 }}>
    <Box sx={{ display: "flex", justifyContent: "space-between", gap: 2, mb: 3, flexWrap: "wrap", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "stretch", sm: "flex-start" } }}>
      <Box>
        <Typography variant="h4" fontWeight={800} sx={{ fontSize: { xs: "1.35rem", sm: "1.75rem" }, letterSpacing: "-0.025em" }}>Ticket payments</Typography>
        <Typography color="text.secondary" sx={{ mt: 0.5, fontSize: "0.9375rem" }}>Review Paystack orders, ticket sales, and payment confirmations.</Typography>
      </Box>
      <Button startIcon={<FileDownloadIcon />} variant="outlined" onClick={exportCsv} sx={{ minHeight: "40px", width: { xs: "100%", sm: "auto" } }}>Export CSV</Button>
    </Box>
    {error && <Paper sx={{ p: 2, mb: 2, color: "#B42318" }}>{error}</Paper>}
    <Box sx={{ display: "flex", gap: 1.5, mb: 2, flexWrap: "wrap", flexDirection: { xs: "column", sm: "row" } }}>
      <TextField size="small" placeholder="Search reference or ticket tier" value={search} onChange={(e) => { setPage(1); setSearch(e.target.value); }} InputProps={{ startAdornment: <SearchIcon fontSize="small" sx={{ mr: 1 }} /> }} sx={{ flex: 1, minWidth: { xs: "100%", sm: 240 } }} />
      <TextField select size="small" label="Status" value={status} onChange={(e) => { setPage(1); setStatus(e.target.value); }} sx={{ minWidth: { xs: "100%", sm: 160 } }}>{statuses.map((value) => <MenuItem key={value || "all"} value={value}>{statusLabel(value)}</MenuItem>)}</TextField>
    </Box>
    <TableContainer component={Paper} sx={{ overflowX: "auto", maxWidth: "100%", width: "100%", borderRadius: "12px", border: "1px solid #E8EAEF" }} elevation={0}>
      <Table sx={{ minWidth: { xs: 800, md: 1000 }, "& .MuiTableCell-root": { whiteSpace: "nowrap" } }}>
        <TableHead><TableRow><TableCell>Reference</TableCell><TableCell>Customer</TableCell><TableCell>Event / Ticket</TableCell><TableCell>Qty</TableCell><TableCell>Amount</TableCell><TableCell>Status</TableCell><TableCell>Date</TableCell><TableCell /></TableRow></TableHead>
        <TableBody>{items.length ? items.map((item) => <TableRow hover key={item.id}><TableCell sx={{ fontFamily: "monospace", fontSize: 12 }}>{item.reference}</TableCell><TableCell><strong>{[item.customer?.firstName, item.customer?.lastName].filter(Boolean).join(" ") || "—"}</strong><br /><Typography variant="caption">{item.customer?.email || "—"}</Typography></TableCell><TableCell>{item.events?.title || "—"}<br /><Typography variant="caption">{item.tier_name}</Typography></TableCell><TableCell>{item.quantity}</TableCell><TableCell>{money(item.amount_kobo)}</TableCell><TableCell><StatusChip status={item.status === "paid" ? "published" : item.status === "failed" ? "archived" : "draft"} label={item.status} /></TableCell><TableCell>{new Date(item.created_at).toLocaleDateString("en-NG")}</TableCell><TableCell><IconButton aria-label="View payment" onClick={() => openPayment(item.id)}><VisibilityOutlinedIcon fontSize="small" /></IconButton></TableCell></TableRow>) : <TableRow><TableCell colSpan={8} align="center">No ticket payments found.</TableCell></TableRow>}</TableBody>
      </Table>
    </TableContainer>
